...
const Navbar = () => {
  ...
  // Effect to update the main content margin when sidebar expands/collapses
  useEffect(() => {
    const mainContent = document.querySelector('.main-content');
    if (mainContent) {
      if (sidebarExpanded) {
        mainContent.style.marginLeft = '168px'; // 42px sidebar width
        mainContent.style.transition = 'margin-left 0.3s ease-in-out';
      } else {
        mainContent.style.marginLeft = '64px'; // 16px sidebar width
        mainContent.style.transition = 'margin-left 0.3s ease-in-out';
      }
    }
  }, [sidebarExpanded]);
  ...
  return (
    <div className={`${darkMode ? 'dark' : ''}`}>
      ...
      <aside
        ...
        light:bg-[#f2eae2] bg-white dark:bg-gray-800 shadow-lg`}
        onMouseEnter={() => setSidebarExpanded(true)}
        onMouseLeave={() => setSidebarExpanded(false)}
      >
        {/* Sidebar header */}
        <div className="h-16 flex items-center px-4 border-b border-gray-200 dark:border-gray-700">
          <Link to="/" className={`text-xl font-bold text-purple-700 dark:text-purple-400 transition-opacity duration-300 ${sidebarExpanded ? 'opacity-100' : 'opacity-0'}`}>
            FinanceTracker
          </Link>
        ...
        {isAuthenticated && user && (
          <div className={`px-4 py-4 border-b border-gray-200 dark:border-gray-700 transition-all duration-300 overflow-hidden ${sidebarExpanded ? 'opacity-100' : 'opacity-0'}`}>
            <div className="flex items-center">
              <div className="h-9 w-9 rounded-full dark:bg-purple-700 bg-purple-200 flex items-center justify-center flex-shrink-0 dark:text-white text-black">
                {user.name.charAt(0).toUpperCase()}
              </div>
              <div className="ml-3">
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Welcome,</p>
                <p className="text-sm font-bold dark:text-gray-900truncate max-w-[140px] ">{user.name}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <nav className="mt-5 px-2 space-y-1">
          ...
          >
            ...
            <span className={`ml-3 whitespace-nowrap transition-opacity duration-300 ${sidebarExpanded ? 'opacity-100' : 'opacity-0'}`}>Home</span>
          </Link>
          ...
          {isAuthenticated ? (
            <>
              ...
              >
                ...
                <span className={`ml-3 whitespace-nowrap transition-opacity duration-300 ${sidebarExpanded ? 'opacity-100' : 'opacity-0'}`}>Dashboard</span>
              </Link>
              ...
              >
                ...
                <span className={`ml-3 whitespace-nowrap transition-opacity duration-300 ${sidebarExpanded ? 'opacity-100' : 'opacity-0'}`}>Transactions</span>
              </Link>
              ...
              >
                ...
                <span className={`ml-3 whitespace-nowrap transition-opacity duration-300 ${sidebarExpanded ? 'opacity-100' : 'opacity-0'}`}>Profile</span>
              </Link>
              ...
              >
                ...
                <span className={`ml-3 whitespace-nowrap transition-opacity duration-300 ${sidebarExpanded ? 'opacity-100' : 'opacity-0'}`}>Logout</span>
              </button>
            </>
          ) : (
            <>
              ...
              >
                ...
                <span className={`ml-3 whitespace-nowrap transition-opacity duration-300 ${sidebarExpanded ? 'opacity-100' : 'opacity-0'}`}>Login</span>
              </Link>
              ...
              >
                ...
                <span className={`ml-3 whitespace-nowrap transition-opacity duration-300 ${sidebarExpanded ? 'opacity-100' : 'opacity-0'}`}>Register</span>
              </Link>
            </>
          )}
          ...
          >
            ...
            <span className={`ml-3 whitespace-nowrap transition-opacity duration-300 ${sidebarExpanded ? 'opacity-100' : 'opacity-0'}`}>
              {darkMode ? 'Light' : 'Dark'}
            </span>
          </button>
        </nav>
